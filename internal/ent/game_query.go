...
type GameQuery struct {
	...
	withGameCategory *CategoryQuery
...
func (gq *GameQuery) QueryGameCategory() *CategoryQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, game.GameCategoryTable, game.GameCategoryColumn),
		)
...
func (gq *GameQuery) Clone() *GameQuery {
	...
	return &GameQuery{
		...
		withGameCategory: gq.withGameCategory.Clone(),
...
// WithGameCategory tells the query-builder to eager-load the nodes that are connected to
// the "game_category" edge. The optional arguments are used to configure the query builder of the edge.
func (gq *GameQuery) WithGameCategory(opts ...func(*CategoryQuery)) *GameQuery {
	query := (&CategoryClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withGameCategory = query
	return gq
}
...
func (gq *GameQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Game, error) {
	...
	if query := gq.withGameCategory; query != nil {
		if err := gq.loadGameCategory(ctx, query, nodes,
			func(n *Game) { n.Edges.GameCategory = []*Category{} },
			func(n *Game, e *Category) { n.Edges.GameCategory = append(n.Edges.GameCategory, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GameQuery) loadGameCategory(ctx context.Context, query *CategoryQuery, nodes []*Game, init func(*Game), assign func(*Game, *Category)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(category.FieldGameID)
	}
	query.Where(predicate.Category(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(game.GameCategoryColumn), fks...))
	}))
